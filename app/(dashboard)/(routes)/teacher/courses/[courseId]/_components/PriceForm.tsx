...
const formSchema = z.object({
    price: z.coerce.number().positive().max(299),//coerce es una funcion que convierte el valor a un tipo de dato especifico
});
...
export const PriceForm = ({initialData, courseId}:PriceFormProps) => {
    ...
    return (
        <div className='mt-6 border bg-slate-100 rounded-md p-4'>
            ...
            {!isEditing && (
                <p className={cn("text-sm mt-2 truncate", !initialData.price && "text-slate-500 italic")}>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 mt-4'>
                        <FormField control={form.control} name='price' render={({field}) => (
                            <FormItem>
                                <FormControl>
                                    <Input {...field} step="0.01" type='number' placeholder='Set a price, not higuer than $299' />
                                </FormControl>
                                <FormMessage/>
                            </FormItem>
                        )}/>
                        <div>
                            {!isValid && (
                                <p className='text-red-500 bg-red-200 border-2 rounded-3xl p-4 font-bold'>Maximum price $299</p>
                            )}
                        </div>
                        ...
                        <div className='flex items-center gap-x-2'>
                            ...
                            {isSubmitting && (
                                <p className='text-slate-500'>Saving... Please wait</p>
                            )}
                        </div>
